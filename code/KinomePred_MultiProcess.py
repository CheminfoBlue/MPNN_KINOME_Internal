...
# params=("AAK1","MEK4","ACVR1B","ABL1(E255K)-phosphorylated","ABL1(F317I)-nonphosphorylated","ABL1(F317L)-nonphosphorylated")
...
def main():
    ...
    script_args_list = [['./data/kinome_data_multiclass_current.csv', '--structure_col', 'Structure', '--activity', p,
                         '--workflow', 'prospective', '--feature_path', './data/KinomeData_ECFP6_FPs_all.csv', '--model_type',
                         'LGB', '--hyp_params', '{"random_state": 30, "class_weight": "balanced"}', '--task_type', 'multiclass',
    ...
    # Calculate the number of available CPU affinity masks
    ...
    for i in range(total_processes):
        args = script_args_list[i % len(script_args_list)]

        # Wait until at least -cpus_per_process- CPUs are available before starting a new process
        while len(processes) >= min(cpus_per_process, num_cores):
            for p in processes:
                if not p.is_alive():
                    p.join()
                    processes.remove(p)

        # Create a Process instance with the corresponding affinity mask
        process = multiprocessing.Process(target=run_script_with_affinity, args=(script_path, args, affinity_masks[i % len(affinity_masks)]))
    ...
    #     # Run the script with different argument sets in parallel
    #     # pool.starmap(run_script, [(script_path, args) for args in script_args_list])

    #     # Set CPU affinity mask for each process
    #     affinity_masks = [list(range(i, min(i + cpus_per_process, num_cores))) for i in range(0, num_cores, cpus_per_process)]
        
    #     # Run the script with different argument sets and CPU affinities in parallel
    #     pool.starmap(run_script_with_affinity, [(script_path, args, mask) for args, mask in zip(script_args_list, affinity_masks)])
