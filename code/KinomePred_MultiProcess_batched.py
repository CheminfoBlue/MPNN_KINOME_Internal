...
print('RUNNING KINOME PREDICTION FOR %d KINASE TARGETS - SYNCHRONOUS'%len(params+tuple(("S(10)", "S(35)"))))
...
def main():
    ...
    #add S-score (S10 & S35) regression tasks
    script_args_list += [['./data/kinome_data_multiclass_current.csv', '--structure_col', 'Structure', '--activity', p,
                         '--workflow', 'prospective', '--feature_path', './data/KinomeData_ECFP6_FPs_all.csv', '--model_type',
                         'LGB', '--hyp_params', '{"random_state": 30}', '--task_type', 'regression',
                         '--checkpoint_path', './results/LGB/%s_model.rds'%p, '--results_path', './results/LGB/'] for p in ["S(10)", "S(35)"]]
